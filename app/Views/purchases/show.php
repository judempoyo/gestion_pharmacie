<div class="max-w-4xl p-6 mx-auto mt-8 bg-white rounded-lg shadow dark:bg-gray-800">
    <div class="flex items-start justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Bon de commande #<?= $purchase->id ?></h1>
            <p class="text-gray-600 dark:text-gray-300">Date: <?= date('d/m/Y', strtotime($purchase->created_at)) ?></p>
        </div>
        <div class="flex space-x-2">
            <a href="<?= PUBLIC_URL ?>purchase/print/<?= $purchase->id ?>" target="_blank" 
               class="px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700">
                Imprimer
            </a>
            <a href="<?= PUBLIC_URL ?>purchase/edit/<?= $purchase->id ?>" 
    ...
    <div class="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2">
        <div class="p-4 border rounded dark:border-gray-600">
            ...
            <p class="text-gray-700 dark:text-gray-300"><?= htmlspecialchars($purchase->supplier->name) ?></p>
            <p class="text-gray-700 dark:text-gray-300"><?= htmlspecialchars($purchase->supplier->phone) ?></p>
        </div>
        <div class="p-4 border rounded dark:border-gray-600">
